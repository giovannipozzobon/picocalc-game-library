...
// --- Inizializzazione ILI9488 ---
void lcd_init(lcd_t *lcd) {
    lcd->width = LCD_WIDTH;
    lcd->height = LCD_HEIGHT;
    lcd->framebuffer = calloc(LCD_WIDTH * LCD_HEIGHT, sizeof(uint16_t));

    // Inizializza SPI a 20 MHz (più sicuro del 40 MHz)
    ...
    // Sequenza di inizializzazione ILI9488P per RGB565
...
void lcd_update_dma(lcd_t *lcd) {
    ...
    // Swap dei byte per big-endian (in-place)
